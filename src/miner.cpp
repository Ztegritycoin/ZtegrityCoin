...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
...

    {
        ...
        LogPrint("ztegritycoin", "CreateNewBlock(): total size %u\n", nBlockSize);
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("ztegritycoinMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock)) {
        ...
        return error("ztegritycoinMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("ztegritycoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("ztegritycoin-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrint("ztegritycoin", "Running ztegritycoinMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
