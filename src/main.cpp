...
#if defined(NDEBUG)
#error "ztegritycoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "ztegritycoin very Signed Message:\n";
...
int64_t GetBlockValue(int nHeight, bool fProofOfStake, uint64_t nCoinAge)
{
    ...
    else
    {
        ...
        } else if (nHeight <= 103680 && nHeight > 2)
        {
            nSubsidy = 25 * COIN;
        } else if (nHeight <= 207360 && nHeight > 103681)
        {
            nSubsidy = 12 * COIN;
        } else if (nHeight > 207361)
        {
            nSubsidy = 6 * COIN;
        } else {
            nSubsidy = 0 * COIN;
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from ztegritycoin
...
{
    RenameThread("ztegritycoin-scriptch");
...
bool RecalculateECASupply(int nHeightStart)
{
    ...
    uiInterface.ShowProgress(_("Recalculating ztegritycoin supply..."), 0);
    while (true) {
        if (pindex->nHeight % 1000 == 0) {
            LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
            int percent = std::max(1, std::min(99, (int)((double)((pindex->nHeight - nHeightStart) * 100) / (chainActive.Height() - nHeightStart))));
            uiInterface.ShowProgress(_("Recalculating ztegritycoin supply..."), percent);
        }
...
#ifdef ENABLE_WALLET
    // If turned on AutoZeromint will automatically convert ztegritycoin to zECA
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // ztegritycoin: We use certain sporks during IBD, so check to see if they are
